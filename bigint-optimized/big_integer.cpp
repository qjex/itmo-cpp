...
big_integer::big_integer(bool sign, storage const &d) {
    data = d;
    this->sign = sign;
    normalize();
}
...
big_integer operator*(big_integer a, big_integer const &b) {
    storage data(a.length() + b.length());
...
void big_integer::div_mod_operation(big_integer const &b, bool is_mod) {
    ...
    storage ans(r.length() - d.length() + 1);
...
big_integer apply_two_complement(big_integer const &a, big_integer const &b,
    ...
    storage data_a = a.is_negative() ? get_inverted_data(a.get_data()) : a.get_data();
    storage data_b = b.is_negative() ? get_inverted_data(b.get_data()) : b.get_data();
...
storage big_integer::get_data() const {
    return data;
}

storage get_inverted_data(storage data) {
